...
# Left shift schedule for the key
SHIFT_SCHEDULE = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]


def initial_permutation(pt):
    """Perform initial permutation on plaintext."""
    return permute(pt, IP)


def final_permutation(ct):
    """Perform final permutation on ciphertext."""
    return permute(ct, FP)


def permute(data, table):
    """Permute the data according to the provided table."""
    return ''.join(data[i - 1] for i in table)


def expansion(data):
    """Expand the data using the E table."""
...
def generate_keys(key):
    """Generate subkeys using the given key."""
    keys = []
    key = permute(key, PC1)
    left = key[:28]
    right = key[28:]
...
def encrypt_block(block, keys):
    ...
    for key in keys:
        ...
        # XOR with key
        xored = ''.join(str(int(expanded[i]) ^ int(key[i])) for i in range(48))
        # Substitution
        substituted = substitution(xored)
        # Permutation
        permuted = permutation(substituted)
        # XOR with left
        xored = ''.join(str(int(permuted[i]) ^ int(left[i])) for i in range(32))
        left = right
        right = xored
...
def encrypt(plaintext, key):
    ...
    ciphertext = ''
    for block in blocks:
        ciphertext += encrypt_block(block, keys)
    return ciphertext
